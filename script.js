(() => {
  ...
  const slideshowViewerEl = document.getElementById("slideshowViewer");
  ...
  const exitGuardState = { portfolioExitGuard: true };
  const miniSlideshows = new Map();
  let activeSlideshowIndex = 0;
  let mainSlideshowTimer = 0;
  ...
  function renderModels() {
    ...
    document.getElementById("modelGrid").innerHTML = (data.models || [])
      .map((model) => {
        ...
        const isSlideshow = isSlideshowModel(model);
        const viewerMarkup = isSlideshow
          ? renderMiniSlideshow(model, heading)
          : `
        ...
        return `
          <article class="model-card">
            <div class="mini-viewer-frame${isSlideshow ? " is-slideshow" : ""}">
              ${viewerMarkup}
            </div>
  ...
  function initMiniSlideshows() {
    document.querySelectorAll("[data-mini-slideshow]").forEach((container) => {
      if (miniSlideshows.has(container)) return;
      const model = findModel(container.dataset.miniSlideshow);
      if (!model) return;
      miniSlideshows.set(container, createSlideshow(container, model, { autoAdvance: true, interval: 2600 }));
    });
  }
  ...
  function bindExitPrompt() {
    if (window.PORTFOLIO_EXIT_PROMPT_READY) return;
    ...
    if (window.history?.pushState) {
      ...
      history.replaceState({ ...currentState, portfolioExitBase: true }, "", location.href);
      history.pushState(exitGuardState, "", location.href);
    ...
    window.addEventListener("beforeunload", (event) => {
      if (allowWebsiteClose || allowNextUnload) return;
      event.preventDefault();
      event.returnValue = "Did you hire me yet?";
    });
  ...
  function showExitPrompt(stage) {
    ...
    exitPrompt.querySelector("button")?.focus();
  }
  ...
  function setActiveModel(modelId) {
    ...
    modelDetails.innerHTML = `
      ...
      <p>${escapeHtml(model.summary)}</p>
      ${model.presentation?.note ? `<p class="viewer-note">${escapeHtml(model.presentation.note)}</p>` : ""}
    `;

    if (isSlideshowModel(model)) {
      switchToSlideshow(model);
      return;
    }

    switchToInteractiveModel();
    if (!mainViewer) return;
  ...
  function switchToInteractiveModel() {
    stopMainSlideshow();
    slideshowViewerEl.onmouseenter = null;
    slideshowViewerEl.onmouseleave = null;
    slideshowViewerEl.ontouchstart = null;
    slideshowViewerEl.hidden = true;
    slideshowViewerEl.innerHTML = "";
    modelViewerEl.hidden = false;
    viewerFrame.classList.remove("is-slideshow-active");
    setViewerToolbarDisabled(false);
  }

  function switchToSlideshow(model) {
    activeSlideshowIndex = 0;
    stopMainSlideshow();
    viewerFrame.classList.remove("has-interacted");
    modelViewerEl.hidden = true;
    slideshowViewerEl.hidden = false;
    viewerFrame.classList.add("is-slideshow-active");
    hideFallback();
    setViewerToolbarDisabled(true);
    renderMainSlideshow(model);
  }

  function renderMainSlideshow(model) {
    const images = getPresentationImages(model);
    if (!images.length) {
      showFallback("Preview unavailable", "Add still images for this confidential model to assets/stills.");
      slideshowViewerEl.innerHTML = "";
      return;
    }
    ...
    slideshowViewerEl.innerHTML = `
      <div class="slideshow-stage">
        <button type="button" class="slideshow-arrow" data-slide-nav="prev" aria-label="Previous view">&lt;</button>
        <figure class="slideshow-figure">
          <img class="slideshow-image" data-main-slide-image src="${escapeAttr(images[0].src)}" alt="${escapeAttr(images[0].alt || getDisplayTitle(model))}" />
        </figure>
        <button type="button" class="slideshow-arrow" data-slide-nav="next" aria-label="Next view">&gt;</button>
      </div>
      <div class="slideshow-footer">
        <span class="slideshow-caption" data-main-slide-caption>View 1 of ${images.length}</span>
        <div class="slideshow-dots" role="tablist" aria-label="${escapeAttr(`${getDisplayTitle(model)} image views`)}">
          ${images
            .map(
              (_, index) =>
                `<button type="button" class="slideshow-dot${index === 0 ? " is-active" : ""}" data-slide-dot="${index}" aria-label="Show view ${index + 1}" aria-pressed="${index === 0 ? "true" : "false"}"></button>`,
            )
            .join("")}
        </div>
      </div>
    ...
    const update = (index) => {
      activeSlideshowIndex = (index + images.length) % images.length;
      const activeImage = images[activeSlideshowIndex];
      const imageEl = slideshowViewerEl.querySelector("[data-main-slide-image]");
      const captionEl = slideshowViewerEl.querySelector("[data-main-slide-caption]");

      if (imageEl) {
        imageEl.src = activeImage.src;
        imageEl.alt = activeImage.alt || getDisplayTitle(model);
      }

      if (captionEl) {
        captionEl.textContent = `View ${activeSlideshowIndex + 1} of ${images.length}`;
      }

      slideshowViewerEl.querySelectorAll("[data-slide-dot]").forEach((dot) => {
        const isActive = Number(dot.dataset.slideDot) === activeSlideshowIndex;
        dot.classList.toggle("is-active", isActive);
        dot.setAttribute("aria-pressed", String(isActive));
      });
    };

    slideshowViewerEl.querySelector('[data-slide-nav="prev"]')?.addEventListener("click", () => update(activeSlideshowIndex - 1));
    slideshowViewerEl.querySelector('[data-slide-nav="next"]')?.addEventListener("click", () => update(activeSlideshowIndex + 1));
    slideshowViewerEl.querySelectorAll("[data-slide-dot]").forEach((dot) => {
      dot.addEventListener("click", () => update(Number(dot.dataset.slideDot)));
    });

    if (images.length > 1) {
      startMainSlideshow(() => update(activeSlideshowIndex + 1), 3200);
      slideshowViewerEl.onmouseenter = () => stopMainSlideshow();
      slideshowViewerEl.onmouseleave = () => startMainSlideshow(() => update(activeSlideshowIndex + 1), 3200);
      slideshowViewerEl.ontouchstart = () => stopMainSlideshow();
    }
  ...
  function isSlideshowModel(model) {
    return model?.presentation?.type === "slideshow";
  }

  function getPresentationImages(model) {
    return Array.isArray(model?.presentation?.images) ? model.presentation.images : [];
  }

  function renderMiniSlideshow(model, heading) {
    const images = getPresentationImages(model);
    const firstImage = images[0];

    return `
      <div class="mini-slideshow" data-mini-slideshow="${escapeAttr(model.id)}">
        <figure class="mini-slideshow-figure">
          <img
            class="mini-slideshow-image"
            data-mini-slide-image
            src="${escapeAttr(firstImage?.src || "")}"
            alt="${escapeAttr(firstImage?.alt || `${heading} still preview`)}"
          />
        </figure>
        <div class="mini-slideshow-badge">Confidential preview</div>
        <div class="mini-slideshow-dots">
          ${images.map((_, index) => `<span class="mini-slideshow-dot${index === 0 ? " is-active" : ""}"></span>`).join("")}
        </div>
      </div>
    `;
  }

  function createSlideshow(container, model, options = {}) {
    const images = getPresentationImages(model);
    const imageEl = container.querySelector("[data-mini-slide-image]");
    const dots = [...container.querySelectorAll(".mini-slideshow-dot")];
    let index = 0;
    let timer = 0;
    ...
    const update = (nextIndex) => {
      ...
      imageEl.alt = images[index].alt || getDisplayTitle(model);
    ...
    if (options.autoAdvance && images.length > 1) {
      timer = window.setInterval(() => update(index + 1), options.interval || 2800);
  ...
  function setViewerToolbarDisabled(disabled) {
    document.querySelectorAll(".viewer-toolbar button").forEach((button) => {
      button.disabled = disabled;
    });
  }

  function startMainSlideshow(step, interval) {
    stopMainSlideshow();
    mainSlideshowTimer = window.setInterval(step, interval);
  }

  function stopMainSlideshow() {
    if (!mainSlideshowTimer) return;
    window.clearInterval(mainSlideshowTimer);
